...
def upgrade() -> None:
    ...
    bind.execute(sa.text("update public.expenses set category='Other' where category is null or category not in ('Food','Fuel','Stay','Transport','Activities','Shopping','Bills','Other')"))

    checks = {row[0] for row in bind.execute(sa.text(
        "select constraint_name from information_schema.table_constraints where table_schema='public' and table_name='expenses' and constraint_type='CHECK'"
    ))}
    ...
    indexes = {row[0] for row in bind.execute(sa.text(
        "select indexname from pg_indexes where schemaname='public' and tablename='expenses'"
    ))}
    ...
    op.alter_column("expenses", "category", server_default=None, schema="public")
